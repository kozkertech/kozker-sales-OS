    ...
    <div className="h-full flex flex-col bg-quiet-bg text-quiet-text overflow-y-auto">
      <div className="h-16 shrink-0 px-6 flex items-center border-b border-quiet-border">
        <div>
          ...
          <span className="font-mono text-xs text-quiet-muted">Human-in-the-loop · nothing acts or sends without you</span>
        </div>
      </div>
      ...
      <div className="p-6 max-w-4xl w-full space-y-8">
        {/* AI agent */}
        <div>
          <h2 className="font-body text-xs uppercase tracking-wider text-quiet-muted mb-3 flex items-center gap-2">
            <Bot size={14} className="text-coral" /> Ask AI to act
          </h2>
          <div className="flex items-end gap-2 bg-quiet-surface border border-quiet-border rounded-sm focus-within:ring-1 focus-within:ring-coral mb-4">
            <textarea
          ...
          {actions.length === 0 ? (
            <p className="font-body text-sm text-quiet-muted">No proposed actions. Ask the AI above to draft some.</p>
          ) : (
            <div className="space-y-2" data-testid="agent-actions">
              {actions.map((a) => (
                <div key={a.id} className="flex items-center gap-3 border border-quiet-border rounded-sm px-4 py-3" data-testid="agent-action">
                  <span className="font-mono text-[10px] uppercase tracking-wider border border-quiet-border px-2 py-0.5 rounded-sm text-quiet-muted shrink-0">
                    {a.type.replace("_", " ")}
                  </span>
                  <span className="flex-1 font-body text-sm text-quiet-text">{a.description}</span>
                  ...
                  <button onClick={() => decideAction(a.id, false)} data-testid="reject-action" className="text-quiet-muted hover:text-quiet-text" title="Reject"><X size={17} /></button>
                </div>
              ))}
            </div>
          )}
        ...
        <div>
          ...
          {messages.length === 0 ? (
            <p className="font-body text-sm text-quiet-muted">No messages waiting. Enroll a contact in a sequence to draft one.</p>
          ) : (
...
function MessageCard({ message, onResolved }) {
  ...
  return (
    <div className="border border-quiet-border rounded-sm p-4" data-testid="message-card">
      <div className="flex items-center gap-2 mb-3">
        {message.channel === "email" ? <Mail size={14} className="text-coral" /> : <MessageCircle size={14} className="text-coral" />}
        <span className="font-body text-sm text-quiet-text">{message.contact_name}</span>
        <span className="font-mono text-xs text-quiet-muted">· {message.to}</span>
        <span className="ml-auto font-mono text-[10px] uppercase tracking-wider text-quiet-muted flex items-center gap-1">
          <Sparkles size={10} className="text-coral" /> {message.sequence_name}
          {message.channel === "whatsapp" && <span className="text-quiet-muted">· mock</span>}
        </span>
      </div>
      {message.channel === "email" && (
        <input value={subject} onChange={(e) => setSubject(e.target.value)} data-testid="message-subject"
          className="w-full bg-quiet-surface border border-quiet-border text-quiet-text font-body text-sm px-3 py-2 mb-2 rounded-sm focus:outline-none focus:ring-1 focus:ring-coral" />
      )}
      <textarea value={body} onChange={(e) => setBody(e.target.value)} rows={4} data-testid="message-body"
        className="w-full bg-quiet-surface border border-quiet-border text-quiet-text font-body text-sm px-3 py-2 rounded-sm focus:outline-none focus:ring-1 focus:ring-coral resize-none leading-relaxed" />
      <div className="flex items-center gap-2 mt-3">
        ...
        <button onClick={reject} data-testid="reject-message"
          className="flex items-center gap-2 bg-quiet-surface hover:bg-quiet-border border border-quiet-border text-quiet-text font-body text-sm px-4 py-2 rounded-sm transition-colors">
          <X size={14} /> Reject
        </button>
        <span className="font-body text-xs text-quiet-muted ml-auto">Edit before sending — you have full control</span>
      </div>
    </div>
  );
}
